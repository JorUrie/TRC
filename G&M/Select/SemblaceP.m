...
Dok = readtable('Data-CM.csv', 'Range', 'C2:C490', 'ReadRowNames', false); % Data
...
PhiCF = importdata('PhiCF-CM-SNAE.csv'); 
PhiCD = importdata('PhiCD-CM-SNAE.csv'); 
...
SCD = semblance(Fecha, Data, PhiCDr, 100); % Conveccion-Difusion
ST = SCF-SCD;
STi = SCD-SCF;
%% Haciendo las graficas
subplot(3,1,1); imagesc(SCD, [-1 1]); axis xy; axis tight; title('Semblance Convection-Diffusion SNAE'); ylabel('Wavelength');
colormap(jet(256));
colorbar
subplot(3,1,2); imagesc(SCF,[-1 1]); axis xy; axis tight; title('Semblance Force Field SNAE'); ylabel('Wavelength'); 
colormap(jet(256));
colorbar
