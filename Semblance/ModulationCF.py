...
# --------------------------------------------------------------------------------------------------
# --------------------------------------------------------------------------------------------------
# Algoritmo para la solucion de Campo de Fuerza
#Asignando valores de las constantes
# Para protones
def CFP(Data, phi):
    #phi=0.1
    E0 = 0.938         #!!ENERGÍA INICIAL
    Az = 1.0           
    Pb = Data #Data[0]/Data #Arreglo de un sólo valor   np.logspace(-1.2,4,1000)
    Betab = Pb/np.sqrt(Pb*Pb+E0*E0*Az*Az)
    Tb = Pb/Betab/Az-E0
    Jb = LISWLP(Pb)
    P = np.sqrt(((Pb**2+(Az*E0)**2)**(0.5))**2-(Az*E0)**2) -phi
    J = Jb*(P/Pb)**2
    Beta = P/np.sqrt(P*P+(E0*Az)**2)
    T = P/Beta/Az-E0
    return P, J, Beta
# --------------------------------------------------------------------------------------------------
# --------------------------------------------------------------------------------------------------
...
PhiCF= DphiCF(Ek, 5.90)[0]
# Campo de Fuerza
PhiCF = DphiCF(Ek, 5.90)[0]
# Guardando los datos modulados y las fechas en un txt
np.savetxt('PhiCF-CM-AATB.csv', PhiCF, delimiter = ' ')
# Print spectrum
